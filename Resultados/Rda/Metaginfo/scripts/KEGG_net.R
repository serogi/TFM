...
kegg2igraph <- function( name.pathway, kgraph, comp.list ){
  ...
  species <- paste0(unlist(strsplit(name.pathway, split = ""))[1:3], 
                    collapse = "")
  if(species == "dme"){
    id_table <- read.table("~/Documents/TFM/scripts_hipathia/local_hipathia/dme_id.csv", header = TRUE, sep = "\t")
    glist <- dme_kegg_to_entrez(glist, id_table)
    V(ig)$genesList <- glist[match(V(ig)$name,kgraph@nodes)]
    V(ig)$genesList[is.na(V(ig)$genesList)] <- "/"
  }
...
# Returns a list of NCBI Gene Ids from a list of Drosophila melanogaster KEGG Ids
dme_kegg_to_entrez <- function(genelist, id_table){
  for(i in 1:length(genelist)){
    for(j in 1:length(genelist[[i]])){
      if(genelist[[i]][j] == "/" | is.na(genelist[[i]][j]) | genelist[[i]][j] == " "){
        next
      }else{
        genelist[[i]][j] <- id_table[which(id_table == genelist[[i]][j]), 2]
      }
    }
  }
  entrezlist <- genelist
